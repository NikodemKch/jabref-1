...
public class SilverBulletHandler {
  ...
  public void listen(EntryChangedEvent changedEvent) {
    ...
    if (Globals.prefs.getBoolean(JabRefPreferences.USE_USELESS_FEATURE)) {
      if (changedEvent.getBibEntry().getTitle().get().equals("no silver bullet")) {
        dialogService.showInformationDialogAndWait(Localization.lang("no_silver_bullet"),
            Localization.lang("no_silver_bullet_content"));
