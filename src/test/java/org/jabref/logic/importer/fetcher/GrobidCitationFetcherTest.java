...
public class GrobidCitationFetcherTest {
    ...
    static GrobidServicePreferences grobidServicePreferences;
    ...
    static BibEntry defaultErrorBibEntry;

    @BeforeAll
    public static void setup() throws ParseException {
        importFormatPreferences = mock(ImportFormatPreferences.class, Answers.RETURNS_DEEP_STUBS);
        fileUpdateMonitor = new DummyFileUpdateMonitor();
        grobidServicePreferences = new GrobidServicePreferences(false, "http://grobid.cm.in.tum.de:8070");
        grobidCitationFetcher = new GrobidCitationFetcher(grobidServicePreferences, importFormatPreferences, fileUpdateMonitor);
        example1AsBibEntry = BibtexParser.singleFromString(example1AsBibtex, importFormatPreferences, fileUpdateMonitor).get();
        example2AsBibEntry = BibtexParser.singleFromString(example1AsBibtex, importFormatPreferences, fileUpdateMonitor).get();
        example3AsBibEntry = BibtexParser.singleFromString(example1AsBibtex, importFormatPreferences, fileUpdateMonitor).get();
        example4AsBibEntry = BibtexParser.singleFromString(example1AsBibtex, importFormatPreferences, fileUpdateMonitor).get();
        defaultErrorBibEntry = BibtexParser.singleFromString(defaultReturnValue, importFormatPreferences, fileUpdateMonitor).get();
    }
    ...
    public void grobidPerformSearchWithInvalidDataTest() {
        ...
        assertEquals(List.of(defaultErrorBibEntry), entries);
